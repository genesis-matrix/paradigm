...
#+TITLE: notes on GeMa // Salt

* synopsis
* advisement
* tasklist
:PROPERTIES:
:ID: 35A0C0C8-3E86-4DCE-B00B-37609B56A192
:END:
[2017-10-27 Fri 05:14]
[[git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::next@{2017-10-27}::1339][git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::next@{2017-10-27}::1339]]
 - context ::
   - [/] overview
   - info/notes
     - srcs
       #+NAME: 35A0C0C8-3E86-4DCE-B00B-37609B56A192-srcs
       ...
       - gdocs, doc-merge 
         #+BEGIN_EXAMPLE
           Salt Execution Flow (Pillar-driven Walkthrough)

           - read top.sls (top.sls is pillar-based so all targeting and state assignment is via the pillar system)
           - encounter pillar references in match specs (performing lookup using minion id to get sls_path)
           - lookup list of states (assigned by minion id)
           - provide ordered list of state_lst
           - iterate over state_lst to get listing of per-minion state.sls_path
           - with the list of per-minion states constructed, the states are sent/returned to the minion
           - the minion will seek to compile the states into low_sls
           - the states will seek to add contextual pillar data (as a dictionary) to a variable called "lookup" by quering pillar data using the state's sls_uuid
           - the pillar will search for a dataset/dictionary at: "lookup:sls_uuid:{{ sls_uuid }}"
           - there are potentially more than one lookup, but the sls_uuid lookup provides the default values for a state before any other context is examined
           - the data here will contain the default values and a list of "lookup_overrides" that may be checked for merge/stack/include
           - FYI: lookups after sls_uuid ("lookup_overrides") use pillarstack to merged on top of, replace or delete, the values in the default dictionary
           - the "lookup_overrides" will be a list of attributes that may be searched on. For example, (where a "lookup_override" includes "G@id") to locate a minion-specific config, looks in the pillar at: "lookup:G@id:myminion:{{ sls_uuid }}"
           - it should be possible to stack levels of specificity for each state according to a potential list of attribute-based overrides
           - Q: should the list of overrides should be the same for all states, so that it doesn't need to be defined per-sls
           - A: Yes. A standard list should be specified, and where a file doesn't exist it should be skipped without causing an error
           - Q: should there be both a "global_lookup_precedence" and a per-sls "sls_lookup_precedence_override"
         #+END_EXAMPLE
       - gdocs, doc-merge
         #+BEGIN_EXAMPLE
           Genesis-Matrix Recommended Infrastructure

            - Component Overview
              - zabbix - monitoring, incident mgmt, ops event source
              - salt - orchestration, config mgmt
              - consul - runtime config, service discovery
              - rkt - container runtime
              - kubernetes - container cluster logic layer
              - glusterfs - simple featureful persistent storage
              - bareos - backups
              - (optional) openfire - chatops
              - (optional) sysdig - diagnostic instrumentation
            - Component Relationships
              - salt ->(installs)=> rkt
              - rkt ->(launches)=> kubernetes
              - kubernetes->(manages)=> services w/i rkt containers
              - salt-
            - Component Requirements and Constraints
              - Kubernetes 
                - Version: 1.3 or greater is required for rkt containers support.
                  - As of [2016-09-12] CentOS repos only package v1.2
...
[[git:~/.extrefs/gema-ctl/ReadMe.org::master@{2015-10-28}::626][git:~/.extrefs/gema-ctl/ReadMe.org::master@{2015-10-28}::626]]
 - context ::
   - [0/0] overview
   - info/notes
     - srcs
 ...
 - procedure :: setup vagrant'd =salt-master=
   - bring up VM of =salt-master=
     #+BEGIN_SRC bash :dir ~/.vault/uris/gema-ctl
     vagrant up salt-master-t1
     #+END_SRC
   - symlink master config to quick-n-dirty vagrant configs
     #+BEGIN_SRC bash :dir ~/.vault/uris/gema-ctl :results output
     vagrant ssh salt-master-t1 -c 'for i in "conf.master.auto_accept..true.conf" "conf.master.file_roots..gemactl-vagrant.conf" "conf.master.pillar_roots..gemactl-vagrant.conf" ;do sudo ln -s /vagrant/etc/salt/conf.d/${i} /etc/salt/master.d/${i} ;done ; sudo systemctl restart salt-master'
     #+END_SRC
...
[[git:~/.vault/uris/gema-ctl/ReadMe-GeMa.org::master@{2016-03-19}::854][git:~/.vault/uris/gema-ctl/ReadMe-GeMa.org::master@{2016-03-19}::854]]
 - context ::
   - [/] overview
   - info/notes
     - srcs
   ...
   - Questions
   - log
*** howto, Bootstrap a Master in GeMa
:PROPERTIES:
:ID: 1E0AE445-76DD-4510-9997-A1EB91092F23
:END:
[2016-03-10 Thu 19:18]
[[git:~/.vault/uris/gema-ctl/ReadMe-GeMa.org::master@{2016-03-10}::1190][git:~/.vault/uris/gema-ctl/ReadMe-GeMa.org::master@{2016-03-10}::1190]]
 - context ::
   - [0/1] overview
...
** on Packer VM Provisioning w/ Salt
:PROPERTIES:
:ID: DF267F2E-F845-4294-B025-C464E0F4F45B
:END:
:LOGBOOK:
:END:
[2016-06-20 Mon 16:55]
[[git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-06-20}::145][git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-06-20}::145]]
 - context ::
   - [/] overview
   - info/notes
     - srcs
       #+NAME: DF267F2E-F845-4294-B025-C464E0F4F45B-srcs
       #+CAPTION: reference list
       | ! | title                   | url                                                    | desc | rowid   |
       |   | title                   | url                                                    | desc | rowid   |
       |---+-------------------------+--------------------------------------------------------+------+---------|
       | / |                         |                                                        |      | <7>     |
       | # | sudo should be optional | https://github.com/saltstack/salt-bootstrap/issues/880 |      | <<24BF37B7-4D64-4B16-8031-9DA0F7E53E3B>> |
       #+TBLFM: $5='(if (and (string= @1$rowid "rowid") (string= $rowid "")) (concat "<<"(org-id-new) ">>") $rowid)
   - Questions
   - log
...
[[git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-02-18}::9][git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-02-18}::9]]
 - context ::
   - [0/0] overview
   - info/notes
     - srcs
       ...
       |---+-------+---------------------------------------------------------------------------------------+------+------------------------------------------|
       | / |       |                                                                                       |      | <7>                                      |
       | # |       | https://github.com/saltstack/pepper                                                   |      | <<F66BE7D9-31D1-44BF-87E7-ECE938F5F258>> |
...
[[git:~/.vault/uris/gema-ctl/ReadMe-GeMa.org::master@{2016-03-19}::1374][git:~/.vault/uris/gema-ctl/ReadMe-GeMa.org::master@{2016-03-19}::1374]]
 - context ::
   - [0/0] overview
   - info/notes
     - srcs
   ...
   - Questions
   - log
*** Leveraging the Saltstack Formula for Salt
:PROPERTIES:
:ID: 53E93EB9-A4C6-4663-886B-97F846CCEB43
:END:
:LOGBOOK:
CLOCK: [2016-05-30 Mon 15:51]--[2016-05-30 Mon 15:52] =>  0:01
:END:
[2016-05-30 Mon 15:51]
[[git:~/.vault/uris/gema-ctl-core/usr/doc/ref-Salt.org::master@{2016-05-30}::501][git:~/.vault/uris/gema-ctl-core/usr/doc/ref-Salt.org::master@{2016-05-30}::501]]
 - context ::
   - [0/0] overview
   - info/notes
     - srcs
       #+NAME: 53E93EB9-A4C6-4663-886B-97F846CCEB43-srcs
       #+CAPTION: reference list
       | ! | title | url                                                | desc | rowid   |
       |   | title | url                                                | desc | rowid   |
       |---+-------+----------------------------------------------------+------+---------|
       | / |       |                                                    |      | <7>     |
       | # |       | https://github.com/saltstack-formulas/salt-formula |      | <<85B8B08C-A9AF-4E69-9922-D202D5A2E2C0>> |
       #+TBLFM: $5='(if (and (string= @1$rowid "rowid") (string= $rowid "")) (concat "<<"(org-id-new) ">>") $rowid)
   - Questions
     - Q: does setting up a master w/ =salt-formula='s =clean= option remove minion keys
       - Q: No. Existing minion keys are preserved.
   - log
...
[[git:~/.vault/uris/gema-ctl/ReadMe-GeMa.org::master@{2016-03-12}::878][git:~/.vault/uris/gema-ctl/ReadMe-GeMa.org::master@{2016-03-12}::878]]
 - context ::
   - [0/0] overview
   - info/notes
     - srcs
     ...
     - on Scenario Deploy
       - (wip) vagrant test
         ...
         |   | minimal-provision |                                                        |   |   |
         |   |                   |                                                        |   |   |
         |   | testing           | conf.master.file_roots..gemactl-vagrant.conf        |   |   |
         |   |                   | conf.master.pillar_roots..gemactl-vagrant.conf      |   |   |
         ...
         - should be able to run
           #+BEGIN_SRC bash
             ...
             salt-call --local state.single pkg.installed "" pkgs="[git,python-pygit2,salt-minion,salt-master]"
             # add bootstrap configs to /etc/salt/master.d if needed
             # restart salt-master, update file cache
             salt-call --local service.restart salt-master ; sleep 5 ; salt-run fileserver.update
             # apply formula-driven state
             salt salt-master-t1 state.sls formula.salt.master
           #+END_SRC
   ...
   - log
     - [2016-03-19 Sat]
       ...
       - NGa creates file =conf.master..scenario-gema-vagrant.conf= to suppliment testing
       ...
       - NGa works to migrate the configs in =conf.master..scenario-gema-vagrant.conf= to the formula config in pillar =by_machine_role.salt-master=
 ...
 - procedure :: vagrant formula testing
   ...
   - create the following symlinks
     ...
     | /vagrant/etc/salt/conf.d/conf.master.pillar_roots..gemactl-vagrant.conf     |
...
[[git:~/.vault/uris/gema-ctl/ReadMe-GeMa.org::master@{2016-03-19}::1374][git:~/.vault/uris/gema-ctl/ReadMe-Gema.org::master@{2016-03-19}::1374]]
 - context ::
   - [0/0] overview
     - the pillar is a simple 1-pass compiled data store, w/o recursion or reference filling
     - tldr, =salt-call state.apply state.data.salt-master.pillar.gema-git-checkout= is the proposed solution
   - info/notes
     - srcs
       ...
       | # |                            | https://docs.saltstack.com/en/latest/ref/pillar/all/salt.pillar.stack.html              |                             | <<AF640EEA-F74D-4EEA-A083-763DFE87DD74>> |
       | # |                            | https://github.com/bbinet/pillarstack/issues/16                                                                                        |                             | <<F6BBA468-5695-46E5-B749-C90B768C0805>> |
       #+TBLFM: $5='(if (and (string= @1$rowid "rowid") (string= $rowid "")) (concat "<<"(org-id-new) ">>") $rowid)
     - on Initial Understanding a/o Discovery
       - objects are the resource being actioned a/o operated on
       - attributes are the key-value sets of promoted labels
       - labels are key-value pairs that decorate /o associate with an object
     - on Generalized Objects, Attributes, and Labels
       #+CAPTION: Objects
       #+NAME: 0B4776CA-985A-40B3-9118-8D5A3BB0B4F6
       |   |         |
       |---+---------|
       | / |         |
       |   | person  |
       ...
       #+CAPTION: Attributes
       #+NAME: 6C69BF8C-E229-4B66-83DD-A67EDB729FF0
       |   |          |
       |---+----------|
       | / |          |
       |   | name     |
       ...
       |   | sla      |
       |   | memberof |
     - on the Simple Pillar Problem
       - the pillar is a powerful and useful tool
       - it's the right place for lots of important data
       - to coherently manage increasingly complex and dynamic infrastructure means managing complex and dynamic data
       - for complex and dynamic data, the need to treat it as a model gains importance
       - the native pillar isn't capable of the multi-pass interpolation needed to resolve interdependent symbols, but Pillarstack is
     - on the Simple Pillar Solution
       - both Pillarstack and Reclass solve this problem, and permit internal resources
       - Pillarstack ships with Salt since version 2016.3
       - they are similar in purpose, but we'll favor Pillarstack in GeMa because of it comes fully integrated out of the box
     - on Accumulating Pillars in Sequence
       |   | ord |        |                   |
       |---+-----+--------+-------------------|
       | / |     |        |                   |
       |   |   1 | GitFS  | simple            |
       |   |   2 | Stack  | complex           |
       |   |   3 | Consul | runtime overrides |
       - the order of Pillar evaluation can be controlled to manage the way in which the values are combined, and ultimately the resulting data set
       - the GitFS pillar must be sufficient to bootstrap a GeMa master, after which a more complex and varied config may be introduced using pillarstack
       - pillarstack , unfortuneately, cannot use the gitfs transport. So must be cloned to the local disk.
       - a local pillar checkout has advantages: use of the pillar_tree system, and use of the pillarstack system from the same source for management of complex and varied pillar data sources
       - it's still preferable to use gitfs because of the ease, tidyness, and multi-applicability of a transparent git repo. If the sought after advantages become available in this form, the proposed local check-out system should be re-evaluated to determine if it's still the best approach.
       - a local pillar checkout presents two challenges, first keeping the information up to date, and second securing the information from unwanted access
       - 
       - the final pillar will be Consul, used as the "runtime config" pillar
     - on Maintaining an Up-To-Date and Secure Pillar on the filesystem
       - securing a local checkout of pillar data is a challenge that
         should not be taken lightly because one should adopt and
         adapt the pillar under the assumption that an unknown volume
         of the secret information ranging in importance from trivial
         and ephemeral to mission-critical and highly-persistent
       - should prevent OS-bypass attacks, such as: accessing the filesystem data from a "Live" or rescue CD
       - should not be unreasonably constrained by volume size, pillar data of several gigs should be as well supported as smaller sizes, and both should be non-problematic
       - should be automatically pulled, either on trigger or on timer, from a change-controlled source
     - on Local Pillar Storage Proposal for Salt Master
       - must be provisioned with situationally adequate swap space, for most purposes this should be no less than 2Gs
       - swap must be set to automaticaly reset on boot then encrypted with a one time, in-memory cipher that is not stored to disk
       - the in-memory filesystem, =tmpfs= must be used at =/srv/pillar=
       - =tmpfs= will retain the pillar data in memory where possible, and swap to disk where it is not, however because =tmpfs= is volatile and the swap is encrypted, this mount will not be suseptible to casual OS-bypass attacks
       - from the Arch wiki's article on "Swap Encryption"
         #+BEGIN_EXAMPLE
           To enable this feature, simply uncomment the line beginning with swap in /etc/crypttab. Change the <device> parameter to the name of your swap device. For example, it will look
           something like this:
           ...
           This will map /dev/sdX# to /dev/mapper/swap as a swap partition that can be added in /etc/fstab like a normal swap. If you had a non-encrypted swap partition before, do not
           forget to disable it - or re-use its fstab entry by changing the device to /dev/mapper/swap. The default options should be sufficient for most usage. For other options see and an
           explanation of each column, see man 5 crypttab as well as point cryptsetup FAQ 2.3.
         #+END_EXAMPLE
       - 
     - on Retreiving SDB into Pillar
       - using SQLite-backed SDB as an example
         #+BEGIN_SRC plantuml
         digraph g {
           sqlite_file -> sqlite_table -> sdb_lookup_macro -> secrets_nodegroup -> integration_feature
         }
         #+END_SRC
       - as of [2017-07-18 Tue] binary SDB data is not explicitly handled by GeMa. To use it, first encode prior to storage in GeMa and decode prior to export. Look for a '_b64bin' variable suffix to indicate a variable containing base64 encoded binary data.
         #+BEGIN_EXAMPLE
           # works: though it relies on the designer to recognize the 'b64bin' variable suffix and decode/handle as appropriate (this is the best fit for now, it's reliable, predicatable and scriptable)
           [root@salt-master-d1 ~]# salt-run sdb.set sdb://sdb_gema_exmpl__00a/binaryexample_b64bin "$(base64 -w0 example_binary_file)"
           True                                                                                                                                                                              

           # wont work: creates error "Failed to load ext_pillar stack: 'utf8' codec can't decode byte 0xd8 in position 2: invalid continuation byte"
           root@salt-master-d1 ~]# salt-run sdb.set sdb://sdb_gema_exmpl__00a/binaryexample '!!binary '"$(base64 -w0 example_binary_file)"                                                     
           True                                                                                                                                                                                 

           # wont work: base64's multi-line output is treated as successive variables
           [root@salt-master-d1 _files]# salt-run sdb.set sdb://sdb_gema_exmpl__00a/binaryexample "$(printf '\!\!binary "%s"' $(base64 example_binary_file))"
           True

           # wont work: '!!binary' type prefix needs to be escaped
           [root@salt-master-d1 _files]# salt-run sdb.set sdb://sdb_gema_exmpl__00a/binaryexample "!!binary '$(base64 example_binary_file)'"
           True

           # wont work: straight binary addition
           [root@salt-master-d1 _files]# salt-run sdb.set sdb://sdb_gema_exmpl__00a/binaryexample "$(cat example_binary_file)"
           True

         #+END_EXAMPLE

     - on Using GeMa labels a/ lookups from Consul
       - 
     - on Nodegroups in GeMa
       ...
       - the config used prefers =nodegroups= to assign pillar variables to machines and thereby manage a/o manipulate them
       - a minion may be a member of any number of =nodegroups=
       - =nodegroups= are defined in salt-master config files, (ex. ".../salt/master.d/conf.master.sdb.sqlite..sdb_gema_exmpl__00a.conf")
       - =nodegroups= are labels that are prefixed with a capitalized "type".
         | type    | type_desc                                                                                                           | example |
         |---------+---------------------------------------------------------------------------------------------------------------------+---------|
         | CONFIGS | assign states, generic config to minions                                                                            |         |
         | SECRETS | assign a set of sensitive info to the pillar under "lookup:minion_id:<minion_id>:secrets:..."                       |         |
         | DEPLOYS | runtime a/o deploy instruction                                                                                      |         |
         | TARGETS | used only for grouping minions aka targeting, no explicit changes but the target may be used in subsequent matching |         |
       - (wip) more on the CONFIGS type
       - (wip) more on the SECRETS type
       - (wip) more on the DEPLOYS type
       - (wip) more on the TARGETS type
   ...
   - Questions
   - log
 - procedure :: howto, use upstream Pillarstack
   - nb
     - https://github.com/bbinet/pillarstack#installation
     - the default value for the =extension_modules= variable is =/var/cache/salt/master/extmods=
     - consequently, Pillarstack's =stack.py= should be added to =/var/cache/salt/master/extmods/pillar/stack.py=, (based on https://docs.saltstack.com/en/latest/topics/development/external_pillars.html#location)
       : mkdir -p /var/cache/salt/master/extmods/pillar && wget https://raw.githubusercontent.com/bbinet/pillarstack/master/stack.py --output-document=/var/cache/salt/master/extmods/pillar/stack.py
     - Q: can pillar modules be added to the state tree as so many other custom module types can
   - 
** on Salt Cloud
...
** on Salt Execution Flow (Pillar-Driven Walkthrough)
** on Requirements-Oriented Delivery a/ Testing
:PROPERTIES:
:ID: BE8C5B48-5630-4E9C-97B7-AA4F19143FC6
:END:
:LOGBOOK:
:END:
[2016-10-22 Sat 16:37]
[[git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-10-22}::1287][git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-10-22}::1287]]
 - context ::
   - [0/0] overview
   - info/notes
     - srcs
       #+NAME: BE8C5B48-5630-4E9C-97B7-AA4F19143FC6-srcs
       #+CAPTION: reference list
       | ! | title | url | desc | rowid   |
       |   | title | url | desc | rowid   |
       |---+-------+-----+------+---------|
       | / |       |     |      | <7>     |
       | # |       |     |      | <<557D0D6C-5A2E-4B0D-8F8A-471EE738B674>> |
       #+TBLFM: $5='(if (and (string= @1$rowid "rowid") (string= $rowid "")) (concat "<<"(org-id-new) ">>") $rowid)
       - example infratest pillar
         #+BEGIN_SRC yaml
       ...
       - ex. infratest pillar
         #+BEGIN_SRC yaml
           ...
           infratest:
         	...
         	exists: true
         #+END_SRC
       - ex. assign infratest assertions via pillar top
         #+BEGIN_SRC yaml
           # pillar/top.sls
   ...
   - Questions
   - log
 - procedure :: setup saltstack-infratest-module
   - nb, this action is not needed when using gema-sls-core because it is already done
   - add the =infratest.py= test to Salt =_modules= directory
     #+BEGIN_SRC bash
     mkdir -fp /srv/salt/_modules ; cd /srv/salt/_modules ; curl -klO 'https://raw.githubusercontent.com/ssplatt/saltstack-infratest-module/master/infratest.py'
     #+END_SRC
...
** on Logging Salt Action Outcomes
:PROPERTIES:
:ID: 2D21528C-BB64-4B1A-96D1-52C4B386D073
:END:
:LOGBOOK:
:END:
[2016-10-23 Sun 22:35]
[[git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-10-23}::1354][git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-10-23}::1354]]
 - context ::
   - [0/0] overview
   - info/notes
     - srcs
       #+NAME: 2D21528C-BB64-4B1A-96D1-52C4B386D073-srcs
       #+CAPTION: reference list
       | ! | title | url                                                                  | desc | rowid   |
       |   | title | url                                                                  | desc | rowid   |
       |---+-------+----------------------------------------------------------------------+------+---------|
       | / |       |                                                                      |      | <7>     |
       | # |       | https://docs.saltstack.com/en/carbon/topics/jobs/external_cache.html |      | <<D25C108E-0438-4813-90E5-286880B193F1>> |
       #+TBLFM: $5='(if (and (string= @1$rowid "rowid") (string= $rowid "")) (concat "<<"(org-id-new) ">>") $rowid)
     - on Initial Understanding a/o Discovery
       - the =syslog= returner for the =master_job_cache= is a sensible way to get started with an audit trail because it requires no setup other than adding the below to the master config
         : master_job_cache: syslog
       - to properly track returns from multiple masters, the following returners are interesting
         |   |                                                                                                                  |
         |---+------------------------------------------------------------------------------------------------------------------|
         | / |                                                                                                                  |
         |   | https://docs.saltstack.com/en/latest/ref/returners/all/salt.returners.pgjsonb.html#module-salt.returners.pgjsonb |
         |   | https://docs.saltstack.com/en/latest/ref/returners/all/salt.returners.odbc.html                                  |
         |   | https://docs.saltstack.com/en/latest/ref/returners/all/salt.returners.mongo_return.html                          |

   - Questions
   - log

** on Operations Security w/ Hubblestack
:PROPERTIES:
:ID: 470C8727-C6D2-41D4-945C-83E3D1ED14DC
:END:
:LOGBOOK:
:END:
[2016-10-23 Sun 04:08]
[[git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-10-23}::1354][git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::master@{2016-10-23}::1354]]
 - context ::
   - [0/0] overview
   - info/notes
     - srcs
       #+NAME: 470C8727-C6D2-41D4-945C-83E3D1ED14DC-srcs
       #+CAPTION: reference list
       | ! | title | url                                                                         | desc | rowid   |
       |   | title | url                                                                         | desc | rowid   |
       |---+-------+-----------------------------------------------------------------------------+------+---------|
       | / |       |                                                                             |      | <7>     |
       | # |       | https://hubblestack.io/                                                     |      | <<561AA4D9-B767-48F0-8003-B8D9444717C9>> |
       | # |       | https://github.com/HubbleStack                                              |      | <<5B4D62D7-AB92-4F3D-BCA8-E7254C78D3AA>> |
       | # |       | https://www.youtube.com/watch?v=r4CGEVOhLiQ                                 | demo | <<E07280B2-6BBD-4CCE-9B17-B74E1818B21B>> |
       | # |       | https://media.readthedocs.org/pdf/hubblestack/documentation/hubblestack.pdf |      | <<13A98D7D-F38D-4913-8D33-00D14AC618DB>> |
       | # |       | https://hubblestack.io/content/2016.7.1/hubble-introduction-2016.7.1.pdf    |      | <<ADF9F0FD-0000-4DCE-A3C0-6379D9696EDF>> |
       | # |       | https://hubblestack.io/content/2016.7.1/hubble-architecture-2016.7.1.pdf    |      | <<DB5A0398-C1BA-4A34-893C-2E4FA18DB4A9>> |
       #+TBLFM: $5='(if (and (string= @1$rowid "rowid") (string= $rowid "")) (concat "<<"(org-id-new) ">>") $rowid)
       - Quick Start for Nova Instructions
         #+BEGIN_EXAMPLE
           ...
           3. salt \* saltutil.sync_all # alternatively and more minimally use saltutil.sync_modules
           4. salt \* hubble.audit
           # should output a catalog of available checks
         #+END_EXAMPLE
     - on Initial Understanding a/o Discovery
       - GeMa uses Hubblestack's GitFS-based install method and otherwise includes requisite software, so that Hubblestack may be used "out-of-the-box"
       - todo: make the gitfs imports to the roots tree more precise
     - on Hubblestack by Module
       |   |        |                           | initial_use               |
       |---+--------+---------------------------+---------------------------|
       | / |        |                           |                           |
       |   | Nova   | Hubble's Auditing System  | =salt-call hubble.audit=  |
       |   | Quasar | Hubble's Reporting System | n/a, pillar config needed |
       |   | Pulsar | Hubble's FIM System       |                           |
       |   | Nebula | Hubble's Insight System   |                           |
       - Quasar is driven by configs found in Pillar data. See =./srv/gema-plr/base/label/service_feature/salt-master/hubblestack/quasar/pillar.example=, (embedded below), for a configuration example.
         #+BEGIN_SRC yaml
         ...
         #+END_SRC
       - Pulsar is also driven by config that lives in the Pillar. To aid in rapid and sane deployments within GeMa, we've copied the default =example.pillar= to the =gema-plr-core= repo, and included it in the Pillar top file for all nodes.
         - Note, the =hubblestack_pulsar_config.yaml= file is referenced from within the pillar, and provides detail about what in particular to monitor.
         - From the project:
           : Every environment will have different needs and requirements, and we understand that, so we've designed Pulsar to be flexible.
       - Nebula
         - pre-req, =salt-call state.apply state.apps.osquery=
   ...
   - Questions
   - log
** on Salt Simple Database (SDB)
:PROPERTIES:
:ID: 5E0B596E-CEA6-4DDF-8DDF-A20C1CDD3A9B
:END:
[2017-06-30 Fri 22:04]
[[git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::next@{2017-06-30}::1509][git:~/.vault/uris/gema-ctl/usr/doc/ref-Salt.org::next@{2017-06-30}::1509]]
 - context ::
   - [0/0] overview
   - info/notes
     - srcs
       #+NAME: 5E0B596E-CEA6-4DDF-8DDF-A20C1CDD3A9B-srcs
       #+CAPTION: reference list
       |!|title|url|desc|rowid|
       | |title|url|desc|rowid|
       |-|-----|---|----|-----|
       ...
       #+TBLFM: $5='(if (and (string= @1$rowid "rowid") (string= $rowid "")) (concat "<<"(org-id-new) ">>") $rowid)
     - on Initial Understanding a/o Discovery
       - prefer to consolidate initial sdb into one sqlite file under different tables
         - Q: is it problematic to use a single sqlite file from multiple held reference in different place in Salt simultaneously
       - the sdb top-level config can have a human-friendly name but every table name should be a uuid (as created by http://man7.org/linux/man-pages/man1/uuidgen.1.html)
       - prefer that the sdb top-level config name be in the general format: "sdb_gema_nga__0a" as "<system>_<paradigm>_<oi>__<iter_int><subver_alpha>"
       - (?) the key name will be the root binding pillar uri
       - as a security concern, since the file is written to the tmpfs filesystem mounted at =/run=, under typical circumstances it lives in memory and is not written to disk
         : tmpfs on /run type tmpfs (rw,nosuid,nodev,seclabel,mode=755)
       - prefer that sdb data gets implicity discovery (rewrite aren't needed to incorporate new data), the data is shared according to nodegroup membership which is bound to the sdb top-level handle name (ex. "sdb_gema_nga__0a")
       - prefer sdb key names that
         - are clear, human-friendly descriptive labels
         - are free from concern for organizational, contextual, uniquely distinguishing information or metadata
         - are free from concern for extreme terseness or particular schema
         - use underscores to delimit words (to ease downstream data handling)
         - (ex.) digital_ocean_api_token
         - note that all concern for key management (lookup, name collision, etc) are handling at the sdb-handle level by way of manipulating the sdb-handle by creating new ones and combining them as needed
       - this secrets data is filtered according to membership in the respective 'SECRETS_*' nodegroup to ultimately be added to the pillar as below
         #+BEGIN_EXAMPLE
         # templatized
         lookup:
           minion_id:
             ...
             exmpl-node-t1:
               secrets:
                 sdb_gema_exmpl__0a:
                   ...
                   entries:
                     { ... }
                   digital_ocean_api_token: 0xdecafc0ffeebad
         #+END_EXAMPLE
       - after processing, sdb data is presented as a jinja dictionary called =secrets_dct=. As a nicety, the following (per-sdb-handle) "special" keys are importable via a seperate =_meta= dictionary, these  should be regarded as optional conventions
         | key            | value                                                                                                |   |
         |----------------+------------------------------------------------------------------------------------------------------+---|
         | name.handle    | the sdb handle name as a string                                                                      |   |
         | name.human     | a human friendly string as a counterpart and suppliment                                              |   |
         | tags           | short descriptive tags                                                                               |   |
         | doc            | description text containing relevant references, guidance and other helpful information as a string. |   |
         | interpolation  |                                                                                                      |   |
       - in this way the sdb handle becomes a secrets set / privilege barrier _and_ those who are entitled by nodegroup membership will see data that is added or reduced without additional changes
       - note the distinction between seeing data and using it. in some cases it may be possible and appropriate to dynamically adjust to added or removed data but this responsibility lay with the pillar designed _not_ with the sdb designer
       - Salt will create the sqlite file if it doesn't already exist, (on tmpfs at =/run/salt/master/.sdb_gema.sqlite=)
         #+BEGIN_EXAMPLE
         #src:https://docs.saltstack.com/en/latest/ref/sdb/all/salt.sdb.sqlite3.html
         The database and table will be created if they do not exist on the first write/set operation.

         [root@salt-master-d1 ~]# ls -l /run/salt/master/.sdb_gema.sqlite
         ls: cannot access /run/salt/master/.sdb_gema.sqlite: No such file or directory
         [root@salt-master-d1 ~]# salt-run sdb.set sdb://sdb_gema_exmpl__0a/test 123
         True
         [root@salt-master-d1 ~]# ls -l /run/salt/master/.sdb_gema.sqlite
         -rw-r--r--. 1 root root 3072 Jul  1 04:22 /run/salt/master/.sdb_gema.sqlite

         #+END_EXAMPLE
       - to create a sdb/sqlite3 file populated with example data is included with gema-ctl for use with the 'sdb_gema_exmpl__00a' handle for demonstration purposes
         | sdb_handle     | key       | value                       | cli_set_cmd                                                                      |
         |----------------+-----------+-----------------------------+----------------------------------------------------------------------------------|
         | sdb_gema_exmpl | discovery | "will-take-us-to-the-stars" | =salt-run sdb.set sdb://sdb_gema_exmpl__00a/discovery will-take-us-to-the-stars= |
       - nb, in Jinja use of the =format()= filter converts a conventional (ascii?) string to a unicode string implicitly even without using interpolation features
         |   | op/filter | unicode_p? | purpose       |
         |---+-----------+------------+---------------|
         |   | format    | True       | interpolation |
         |   | string    | True       | casting       |
         |   | '~'       | True       | joining       |
         |   | '+'       | False      | joining       |
         |   | join      | False      | joining       |
       - nb, the filters available to Pillarstack is not exactly the same as the Salt state system
         - for example, the filter 'json' is available in the state system but not in pillarstack
         - found this was fixed in upstream pillarstack ([[https://github.com/bbinet/pillarstack/commit/03e1207e304c0b9d88f72cf7a0c185590b0a7090][commit:03e1207]]) which is not yet (as of [2017-07-04 Tue]) merged into the latest Salt release.
         - as remedy a/o work-around, we'll pull a more recent =stack.py= from the Pillarstack project's git. Instructions here: https://github.com/bbinet/pillarstack#installation
         - side-note, this work-around should be removed once a reasonably venerable Saly release includes a pillarstack with these needed changes
         


     - on SDB/SQLite key retrieval
       - because SDB doesn't has only =get=, =set=, and =delete=, but no =list= method, we'll need to introspect salt config data in order to get the listing of keys for each SDB handle
       - the sqlite3 sdb table column names are "key", and "value"
   - Questions
     - Q: how gracefully does the salt master handle unavailable SDB modules
       - b/c I want these to be bootstrap config, it's expected that they'll no longer be needed at some point after initial use
       - the config option =create_table= would seem to mitigate part of the worry though the data would still be absent
     - Q: does SDB automatically create any non-existent needed parent keys needed to set nested data
       - A: Yes, kind of. Note well, sdb provides flat k-v lookups so "nesting" is an illusion, created by using logically similar key that is not reflected in the values.
       - That said, objects (ex. JSON) can be stored in the value they just aren't implicitly inter-related with data at other keys
     - Q: is there a way to list the keys in an SDB config
   - log
 - procedure :: common operations for SDB
   - discover and list keys
     #+BEGIN_EXAMPLE
       [root@salt-master-d1 ~]# salt-call sqlite3.tables /run/salt/master/.sdb_gema.sqlite
       local:
           |_
     	- 5957620F-EA24-431A-8A03-4DFBDC3FF79C
       [root@salt-master-d1 ~]# salt-call sqlite3.fetch /run/salt/master/.sdb_gema.sqlite 'select * from "5957620F-EA24-431A-8A03-4DFBDC3FF79C";' --out=json
       {
           "local": [
               [
                   "test",
                   "<read-write buffer ptr 0x37606e8, size 1 at 0x37606a8>"
               ],
               [
                   "test/of/a/deeply/nested/structure",
                   "<read-write buffer ptr 0x3760778, size 1 at 0x3760738>"
               ]
           ]
       }
     #+END_EXAMPLE
 - procedure :: setup initial =sdb_gema.sqlite=
   - nb
     - this procedure will use an example sdb/sqlite config called "sdb_gema_exmpl__0a"
     - "sdb_gema_exmpl__0a" will use the GeMa paradigm to provide data to the pillar
     - this procedure will demonstrate the steps needed to seed a salt-master with secrets data to subsequently bootstrap into a trusted configuration
     - as an organizational aid, an entry called '__gema_sdb' will contain a dictionary to be updated with the below info
       #+BEGIN_EXAMPLE
       __gema_sdb:
         _title: <
       <some_entry_key>:

         <payload_dtype>: (string|mapping|list)
         <payload>: ...

       #+END_EXAMPLE
   - add the below config file to '.../master.d/conf.master.sdb.sqlite..sdb_gema_exmpl__0a.conf'
     #+BEGIN_EXAMPLE
       # set chown=root:root chmod=0600
       sdb_gema_exmpl__0a:
         driver: sqlite3
         database: /run/salt/master/.sdb_gema.sqlite
         table: "6B3CD61E-DC22-400C-9F71-4F3ABCB839E0"
         create_table: True
     #+END_EXAMPLE
   - with this config in place, the file is created when data is inserted so we'll demonstrate that now
** on TDD w/ Salt Policy
:PROPERTIES:
:ID: 16A39CF7-762D-4CE8-9C6A-BC092EBF0529
:END:
[2018-12-20 Thu 05:30]
[[git:~/.extrefs/paradigm/refs/ref-Salt.org::master@{2018-12-20}::1803][git:~/.extrefs/paradigm/refs/ref-Salt.org::master@{2018-12-20}::1803]]
 - context ::
   - [/] overview
   - info/notes
     - srcs
       #+NAME: 16A39CF7-762D-4CE8-9C6A-BC092EBF0529-srcs
       #+CAPTION: reference list
       | ! | title | url                                        | desc | rowid                                    |
       |   | title | url                                        | desc | rowid                                    |
       |---+-------+--------------------------------------------+------+------------------------------------------|
       | / |       |                                            |      | <7>                                      |
       | # |       | https://github.com/saltstack/pytest-salt   |      | <<5D8C798F-8B7C-4E21-9D28-C5B9530ECDF2>> |
       | # |       | https://github.com/saltstack/kitchen-salt/ |      | <<FB8A533C-8612-40FB-9AB7-ABC258BD95D6>> |
       | # |       | https://github.com/saltstack/salt-testing  |      | <<D6426696-2F66-4565-A580-E50078E95ECD>> |
       | # |       |                                            |      |                                          |
       #+TBLFM: $5='(if (and (string= @1$rowid "rowid") (string= $rowid "")) (concat "<<"(org-id-new) ">>") $rowid)
   - Questions
   - log
 - procedure :: (wip) run =kitchen-salt= (on Debian)
   - start and enter a Debian docker container
     #+begin_src bash
     docker run -it --run -v $(pwd):/app debian
     #+end_src
   - (opt.) clone a project that uses =kitchen-salt= and =cd= to that project directory
   - add pre-req system packages
     #+begin_src bash
     apt update
     apt install ruby ruby-dev ruby-ffi build-essential
     #+end_src
   - install pre-req ruby packages from =Gemfile=
     #+begin_src bash
     bundler install --path $(path)/.ruby/bundle
     # gem install -g
     #+end_src
